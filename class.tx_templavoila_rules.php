...
#debug ($statusArr,'statusArr',__LINE__,__FILE__);
	...
	function checkRulesForElement_parseEL($rulePart, &$childRecords, &$statusArr) {
		...
		if (count($childRecords)) {
			$this->lastParsedRecord = $childRecords[0];
		}
...
#		$this->statusMerge ($statusArr, $tmpStatusArr);
	...
	function checkRulesForElement_parseALT($rulePart, &$childRecords, &$statusArr, $constants) {
		...
		if ($altStatusArr['ok'] && ($statusArr['ok'])) {
			$statusArr['ok'] = TRUE;
		}
		if (!$altStatusArr['ok']) {
			$statusArr['ok'] = FALSE;
		...
		if ($min == $max) {
			switch ($min) {
				...
				case 999:
					...
					break;
				default:
					$description = intval($min) . ' times ';
					break;
			}
		} elseif ($min == 0) {
			switch ($max) {
				case 1:
					$description = 'maybe one ';
					break;
				case 999:
					$description = 'any number of ';
					break;
				default:
					$description = 'up to ' . intval($max) . ' ';
					break;
			}
		} elseif ($min > 0) {
			switch ($max) {
				...
				if ($this->isElement($regex[$pos])) { // Element (ie. a-z A-Z and '.')
					...
					if ($elements) {
						if ($regex[$pos] == ']') {
							...
							debug('Parse error: ] expected at end of class definition');
						}
					} else {
						debug('Parse error: At least one element expected in class definition');
					}
				}
				$pos++;
			}
		...
		if (strpos(' *?+{', $quantifier[$pos + 1])) {
			switch ($quantifier[$pos + 1]) {
				...
				case '{': // Quantifier enclosed in curly braces
					...
					if ($quantifier[$pos + 1] != '}') {
						debug('Parse error! Expected \'}\' at this point');
					}
					break;
				default:
					debug('Parse error! Unexpected token: \'' . $quantifier[$pos + 1] . '\'');
					$ok = 0;
			}
		}
	}
...
#debug (array ('statusArr'=>$statusArr, 'newStatusArr' => $newStatusArr, 'doAND'=>$doAND, 'ANDed'=>($oldOK && $newStatusArr['ok'])),'statusMerge',__LINE__);
			...
			if ($doAND) {
				if (!isset ($statusArr['ok']) || !isset ($newStatusArr['ok'])) {
					if ($statusArr['ok'] == NULL) {
						$statusArr['ok'] = TRUE;
					}
					if ($newStatusArr['ok'] == NULL) {
						$newStatusArr['ok'] = TRUE;
					}
				}
