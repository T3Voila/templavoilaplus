 ...
 *  Copyright notice
 *
 *  (c) 2003, 2004, 2005  Kasper Sk�rh�j (kasper@typo3.com) / Robert Lemke (robert@typo3.org)
 *  All rights reserved
 *
...
class tx_templavoila_xmlrelhndl {
	...
	function pasteRecord($pasteCmd, $source, $destination) {
		...
		if ($sourceStr) {
			...
			switch ($pasteCmd) {
				case 'copy' :
					$this->templavoilaAPIObj->copyElement($sourcePointer, $destinationPointer);
					break;
				case 'copyref':
					$this->templavoilaAPIObj->copyElement($sourcePointer, $destinationPointer, FALSE);
					break;
				case 'localcopy':
					$this->templavoilaAPIObj->copyElement($sourcePointer, $sourcePointer);
					$this->templavoilaAPIObj->unlinkElement($sourcePointer);
					break;
				case 'cut':
					$this->templavoilaAPIObj->moveElement($sourcePointer, $destinationPointer);
					break;
				case 'ref':
					$this->templavoilaAPIObj->referenceElement($sourcePointer, $destinationPointer);
					break;
				case 'unlink':
					$this->templavoilaAPIObj->unlinkElement($sourcePointer);
					break;
				case 'delete':
					$this->templavoilaAPIObj->deleteElement($sourcePointer);
					break;
			}
