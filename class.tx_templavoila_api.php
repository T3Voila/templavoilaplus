 ...
 *  231:     function insertElement_setElementReferences ($destinationPointer, $uid)
 *  253:     function moveElement ($sourcePointer, $destinationPointer)
 *  268:     function moveElement_setElementReferences ($sourcePointer, $destinationPointer)
 *  284:     function copyElement ($sourcePointer, $destinationPointer, $copySubElements = TRUE)
 *  299:     function localizeElement ($sourcePointer, $languageKey)
 *  354:     function referenceElement ($sourcePointer, $destinationPointer)
 *  371:     function referenceElementByUid ($uid, $destinationPointer)
 *  387:     function unlinkElement ($sourcePointer)
 *  400:     function deleteElement ($sourcePointer)
 *
 *              SECTION: Processing functions (protected)
 *  425:     function process ($mode, $sourcePointer, $destinationPointer = NULL, $onlyHandleReferences = FALSE)
 *  482:     function process_move ($sourcePointer, $destinationPointer, $sourceReferencesArr, $destinationReferencesArr, $sourceParentRecord, $destinationParentRecord, $elementRecord, $onlyHandleReferences)
 *  551:     function process_copy ($sourcePointer, $destinationPointer, $sourceReferencesArr, $destinationReferencesArr, $sourceParentRecord, $destinationParentRecord, $elementUid)
 *  588:     function process_copyRecursively ($sourcePointer, $destinationPointer, $sourceReferencesArr, $destinationReferencesArr, $sourceParentRecord, $destinationParentRecord, $elementUid)
 *  627:     function process_reference ($destinationPointer, $destinationReferencesArr, $elementUid)
 *  643:     function process_unlink ($sourcePointer, $sourceReferencesArr)
 *  660:     function process_delete ($sourcePointer, $sourceReferencesArr, $elementUid)
 *
 *              SECTION: Flexform helper functions (public)
 *  703:     function flexform_getValidPointer ($flexformPointer)
 *  748:     function flexform_getPointerFromString ($flexformPointerString)
 *  787:     function flexform_getStringFromPointer ($flexformPointer)
 *  820:     function flexform_getRecordByPointer ($flexformPointer)
 *  844:     function flexform_getPointersByRecord ($elementUid, $pageUid)
 *  869:     function flexform_getElementReferencesFromXML($flexformXML, $flexformPointer)
 *  903:     function flexform_getListOfSubElementUidsRecursively ($table, $uid, &$recordUids)
 *  946:     function flexform_getFlexformPointersToSubElementsRecursively ($table, $uid, &$flexformPointers)
 *
 *              SECTION: Flexform helper functions (protected)
 * 1011:     function flexform_insertElementReferenceIntoList($currentReferencesArr, $position, $elementUid)
 * 1051:     function flexform_removeElementReferenceFromList($currentReferencesArr, $position)
 * 1073:     function flexform_storeElementReferencesListInRecord ($referencesArr, $destinationPointer)
 *
 *              SECTION: Data structure helper functions (public)
 * 1114:     function ds_getFieldNameByColumnPosition ($contextPageUid, $columnPosition)
 * 1168:     function ds_getExpandedDataStructure ($table, $row)
 * 1205:     function ds_getAvailableTORecords ($pageUid)
 *
 *              SECTION: Get content structure of page
 * 1247:     function getContentTree($table, $row, $includePreviewData=TRUE)
 * 1277:     function getContentTree_element($table, $row, &$tt_content_elementRegister, $prevRecList='')
 * 1381:     function getContentTree_processSubContent($listOfSubElementUids, &$tt_content_elementRegister, $prevRecList)
 * 1430:     function getContentTree_processSubElements($flexformContentArr, $tree, $sheetKey, $lKey, $fieldKey, $vKey, $fieldData, $row)
 * 1463:     function getContentTree_getLocalizationInfoForElement($contentTreeArr, &$tt_content_elementRegister)
 *
 *              SECTION: Miscellaneous functions (protected)
 * 1536:     function setTCEmainRunningFlag ($flag)
 * 1548:     function getTCEmainRunningFlag ()
 * 1559:     function getStorageFolderPid($pageUid)
 * 1577:     function loadWebsiteLanguages()
 *
 * TOTAL FUNCTIONS: 43
