...
function sortable_hideRecord(it, command) {
	...
	new Effect.Fade(it, {duration: 0.5, afterFinish: sortable_hideRecordCallBack})
}
...
function sortable_unlinkRecord(pointer, id, elementPointer) {
	new Ajax.Request("index.php?" + sortable_linkParameters + "&ajaxUnlinkRecord=" + escape(pointer), {onSuccess: function(response) {
	...
	new Effect.Fade(id, {duration: 0.5, afterFinish: sortable_unlinkRecordCallBack})
}
function sortable_unlinkRecordSidebarCallBack(pointer) {
	...
	$("tx_templavoila_mod1_sidebar-bar").morph({height: innerHeight + "px"}, {duration: 0.1, afterFinish: function() {
...
function sortable_updateItemButtons(el, position, pID) {
	...
	el.childElements().each(function(node) {
		if (node.nodeName == "A" && node.href) {
			...
			if (node.childElements() && node.className != "tpm-subelement-table") {
...
function tv_createSortable(s, containment) {
	...
	Position.prepare();
	Sortable.create(s, {tag: "div", ghosting: false, format: /(.*)/, handle: "sortable_handle", scroll: "typo3-docbody", scrollSpeed: 30, dropOnEmpty: true, constraint: false, containment: containment, onChange: sortable_change, onUpdate: sortable_update})
};