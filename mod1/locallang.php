<?php
/**
 * Language labels for module "web_txtemplavoilaM1"
 *
 * $Id$
 * 
 * This file is detected by the translation tool.
 */

$LOCAL_LANG = Array (
	"default" => Array (
		"title" => "Page",
		'title_editpage' => 'Edit the page',
		'createnewpage_title' => 'Create a new page',
		'createnewpage_introduction' => '',
		'createnewpage_hide_header' => 'Hide the page',
		'createnewpage_hide_description' => 'If checked, the created page will be initially hidden from the public.',
		'createnewpage_pagetitle_header' => 'Page title',
		'createnewpage_pagetitle_description' => 'Entering a page title is required and helps you to identify your page later.',
		'createnewpage_templateobject_header' => 'Layout',
		'createnewpage_templateobject_description' => 'Please choose one of the layouts below or just take the default layout.',
		'createnewpage_submitlabel' => 'Create the page',
		'createnewpage_templateobject_createemptypage' => 'Create an empty page based on this template:',
		'createnewpage_templateobject_createpagewithdefaultcontent' => 'Create a page with default content based on this template:',
		'template_nodescriptionavailable' => 'No description available.',
		'view_basic' => 'Basic view',
		'view_tree' => 'Tree view',
		'view_fancy' => 'Special fancy view',
	),
	"dk" => Array (
	),
	"de" => Array (
	),
	"no" => Array (
	),
	"it" => Array (
	),
	"fr" => Array (
	),
	"es" => Array (
	),
	"nl" => Array (
	),
	"cz" => Array (
	),
	"pl" => Array (
	),
	"si" => Array (
	),
	"fi" => Array (
	),
	"tr" => Array (
	),
	"se" => Array (
	),
	"pt" => Array (
	),
	"ru" => Array (
	),
	"ro" => Array (
	),
	"ch" => Array (
	),
	"sk" => Array (
	),
	"lt" => Array (
	),
	"is" => Array (
	),
	"hr" => Array (
	),
	"hu" => Array (
	),
	"gl" => Array (
	),
	"th" => Array (
	),
	"gr" => Array (
	),
	"hk" => Array (
	),
	"eu" => Array (
	),
	"bg" => Array (
	),
	"br" => Array (
	),
	"et" => Array (
	),
);
?>